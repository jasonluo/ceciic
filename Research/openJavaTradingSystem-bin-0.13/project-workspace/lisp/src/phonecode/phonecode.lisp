...
(defun main (&key (dict "dict") (nums "nums") (dict-size 100))
  ...
  (with
   open-file in = (nums :direction :input :if-does-not-exist :error)
   do
...
(defun load-dictionary (file size)
  ...
  (with
   var table    = (make-hash-table :test #'eql :size size)
   open-file in = (file :direction :input :if-does-not-exist :error)
   do
...
;;;(load "test.lisp")
;;;(main :dict "phonecode-orig/test.w" :nums "phonecode-orig/test.t")
;;;(main :dict "phonecode-orig/woerter2" :nums "phonecode-orig/z1000.t")