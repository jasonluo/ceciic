...
;Configured for Common Lisp by scmxlate, v 0L
;(c) Dorai Sitaram
...
(defun pregexp-error (where &optional how &rest  what)
  (princ "Error: ")
  (princ where)
  (terpri)
  (princ how)
   ...
   (loop
    (multiple-value-setq (pieces i)
     (let ((pieces pieces) (i i))
      (catch 'loop!
       (return
        (if (>= i n) (list (cons ':seq (nreverse pieces)) i)
        ...
        (let ((i i) (r r))
         (catch 'loop!
          (return
           (if (>= i n)
            (list
             (let ((|Scheme-to-CL-3| (concatenate 'string (nreverse r))))
                          ...
                          (and
                           (let ((|Scheme-to-CL-9| c))
                            (or (char= |Scheme-to-CL-9| #\space)
                             ...
                             (not (graphic-char-p |Scheme-to-CL-9|))))
                           (not *pregexp-space-sensitive?*))
                          (loop! (+ i 1))
                          (if (char= c #\?)
           ...
           (if
            ...
            (if (and (char= c #\,) (= reading 1)) (loop! p q (+ k 1) 2)
             (if (char= c #\})
              (let
               ((p
                 (let ((|Scheme-to-CL-11| (concatenate 'string (nreverse p))))
                  (if (position #\: |Scheme-to-CL-11| :test #'char=) nil
                   (let
                    ((|Scheme-to-CL-12|
                      (read-from-string |Scheme-to-CL-11| nil)))
                    (if (numberp |Scheme-to-CL-12|) |Scheme-to-CL-12| nil)))))
                (q
                 (let ((|Scheme-to-CL-13| (concatenate 'string (nreverse q))))
           ...
           ((#\-)
            (let ((c-prev (car r)))
             (if (characterp c-prev)
              (loop! (cons (list ':char-range c-prev (char s (+ i 1))) (cdr r))
               (+ i 2))
              (loop! (cons c r) (+ i 1)))))
           ((#\[)
            (if (char= (char s (+ i 1)) #\:)
    ...
    (multiple-value-setq (s k)
     (let ((s s) (k k))
      (catch 'loop!
       (return
        (if (null s) nil
         (if (= k i) (car s) (if t (loop! (cdr s) (+ k 1)))))))))))))
     ...
     (if (eql re ':bos) (if (= i start) (funcall sk i backrefs) (funcall fk))
      (if (eql re ':eos) (if (>= i n) (funcall sk i backrefs) (funcall fk))
       (if (eql re ':empty) (funcall sk i backrefs)
        (if (eql re ':wbdry)
         (if (pregexp-at-word-boundary? s i n) (funcall sk i backrefs)
          (funcall fk))
         (if (eql re ':not-wbdry)
          ...
          (if (and (characterp re) (< i n))
           (if
            (funcall (if case-sensitive? #'char= #'char-equal) (char s i) re)
               ...
               ((:neg-char)
                (if (>= i n) (funcall fk)
                 (sub (cadr re) i backrefs
                  #'(lambda (i1 backrefs1)
		      (declare (ignorable backrefs1 i1))
		      (funcall fk))
                ...
                (labels
                 ((loup-or (res)
                   (if (null res) (funcall fk)
                    (sub (car res) i backrefs
                     #'(lambda (i1 backrefs1)
                        (or (funcall sk i1 backrefs1) (loup-or (cdr res))))
                     #'(lambda nil (loup-or (cdr res)))))))
                 (loup-or (cdr re))))
               ((:backref)
                (let ((backref (pregexp-list-ref backrefs (cadr re))))
                 (if backref
                  (pregexp-string-match (subseq s (car backref) (cdr backref))
                   s i n #'(lambda (i) (funcall sk i backrefs)) fk)
                  (funcall sk i backrefs))))
               ((:sub)
                (let ((sub-backref (cons i i)))
                 (let ((backrefs (append backrefs (list sub-backref))))
                  (sub (cadr re) i backrefs
                   #'(lambda (i1 backrefs1)
		       (declare (ignorable backrefs1))
		       (rplacd sub-backref i1)
               ...
               ((:lookbehind)
                (let ((n-actual n)) (setq n i)
                 (let
                  ((found-it?
                    (sub
                     (list ':seq '(:between nil 0 nil :any) (cadr re) ':eos) 0
                     backrefs #'list #'(lambda nil nil))))
                  (setq n n-actual)
                  (if found-it? (funcall sk i backrefs) (funcall fk)))))
               ((:neg-lookbehind)
                (let ((n-actual n)) (setq n i)
               ...
               ((:no-backtrack)
                (let
                 ((found-it?
                   (sub (cadr re) i backrefs #'list #'(lambda nil nil))))
                 (if found-it? (funcall sk (car found-it?) (cadr found-it?))
                  (funcall fk))))
                 ...
                 (let ((p (caddr re)))
                  (let ((q (cadddr re)))
                   (let ((re (car (cddddr re))))
                    (let ((subpat? (and (consp re) (eql (car re) ':sub))))
                     (labels
                           ...
                           ((loup-q (k i cbackrefs)
                             (let
                              ((fk
                                #'(lambda nil
                                   (funcall sk i
                                    (if (eql cbackrefs 'no-match-yet)
                                     (if subpat? (append backrefs (list nil))
                                      backrefs)
                                     cbackrefs)))))
             ...
             #'(lambda (y)
                (let ((jk (car y)))
                 (let ((j (car jk)) (k (cdr jk)))
                  (if (= j k) (loop! (+ k 1) (cons (subseq str i (+ j 1)) r) t)
...
(export '(pregexp-match pregexp-split pregexp-replace pregexp-replace*))
